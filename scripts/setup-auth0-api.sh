...
SPA=$(curl -s -X POST "https://${DOMAIN}/api/v2/clients" \
  ...
  -d '{"name":"AccuTrac Web","app_type":"spa","callbacks":["http://localhost:4200","https://accutrac.org"],"allowed_logout_urls":["http://localhost:4200","https://accutrac.org"],"web_origins":["http://localhost:4200","https://accutrac.org"],"grant_types":["authorization_code","implicit"],"token_endpoint_auth_method":"none","oidc_conformant":true}')
...
API=$(curl -s -X POST "https://${DOMAIN}/api/v2/resource-servers" \
  -H "Authorization: Bearer ${TOKEN}" \
  -H "Content-Type: application/json" \
  -d '{"name":"AccuTrac API","identifier":"https://api.accutrac.org","signing_alg":"RS256","token_lifetime":28800}')
...
if [ -n "$CONN_ID" ] && [ "$CONN_ID" != "$CONNS" ]; then
  ...
  echo "Google connection enabled for AccuTrac Web"
else
...
echo "Audience: https://api.accutrac.org"
