...
import { BatchFacade } from '../state/batch.facade';
...
@Component({
  selector: 'app-shared-create-batch',
  standalone: true,
      ...
      <form (ngSubmit)="onSubmit()" class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        <div class="p-6 sm:p-8 space-y-6">
          <div>
            ...
            <input type="text" [(ngModel)]="batchNumber" name="batchNumber" required
          ...
          <div>
            ...
            <select [(ngModel)]="mineralType" name="mineralType" required
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" [(ngModel)]="originCountry" name="originCountry" required
                placeholder="e.g. CD" maxlength="3"
                class="w-full px-4 py-2.5 border border-slate-300 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 placeholder:text-slate-400 transition-shadow" />
            </div>
            <div>
              <label class="block text-sm font-semibold text-slate-700 mb-1.5">Mine Site</label>
              <input type="text" [(ngModel)]="mineSite" name="mineSite" required
          ...
          <div>
            ...
            <input type="number" [(ngModel)]="estimatedWeight" name="estimatedWeight" required
...
export class SharedCreateBatchComponent {
  ...
  onSubmit() {
    if (!this.batchNumber || !this.mineralType || !this.originCountry || !this.mineSite || this.estimatedWeight == null) return;
